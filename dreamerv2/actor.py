...
class Actor(common.Module):

  def __init__(self, pid, envs, config, step, **kwargs):
    self._pid = pid
    self._envs = envs
    self._kwargs = kwargs
    self._act_spaces = [env.act_space for env in envs]
    self.act_space = envs[0].act_space['action']
    self.step = step
    self.config = config
    self.tfstep = tf.Variable(int(self.step), tf.int64)
    self.wm = WorldModel(config, envs[0].obs_space, self.tfstep)
  ...
  def rollout(self, variables, steps=0, episodes=0, mode='train'):
    ...
    while step < steps or episode < episodes:
      print("agent {}, step: {}".format(self._pid, step))
      obs = {
      ...
      actions, self._state = self.policy(obs, self._state, mode=mode)
      actions = [
  ...
  def policy(self, obs, state=None, mode='train'):
    ...
    if mode == 'eval':
      ...
      if hasattr(self.act_space, 'n'):
        dist = common.dists.OneHotDist(tf.zeros(self.act_space.n))
      else:
        dist = tfd.Uniform(self.act_space.low, self.act_space.high)
        dist = tfd.Independent(dist, 1)
      action = dist.sample(len(obs['is_first']))
      noise = self.config.expl_noise
